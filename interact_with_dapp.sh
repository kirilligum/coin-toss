...
# $TRUST_AND_TRAIN_L1_ADDRESS: This might be the address of a smart contract deployed on the blockchain. Given the name, it could be related to a coin toss game or function.
...
# 6. Check the value of the last conversation `getConversation(uint256)` with 'current_conversation_id' as an argumnent in the `TrustAndTrain` smart contract to see the persisted result in layer-1 due to the voucher execution. Default is 0 for 'current_conversation_id'; change 'current_conversation_id' appropriatly

docker run --rm --net="host" ghcr.io/foundry-rs/foundry "cast call --rpc-url $RPC_URL $TRUST_AND_TRAIN_L1_ADDRESS \"getConversation(uint256)\" 0"
